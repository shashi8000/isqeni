...
public function getOrderdata($ar,$cat){
...
//////////////////
      error_reporting(0);
if(isset($ar['pay']) && $ar['pay']=='' && $ar['status']=='' && $ar['from']=='' &&  $ar['to']=='' ){


}
else if(!empty($ar) && $ar['pay']=='' && $ar['status']=='' && $ar['from']=='' &&  $ar['to']!='' ){
 $this->db->where("t3.order_datetime <= '".$ar['to']."'");
  
}
else if(!empty($ar) && $ar['pay']=='' && $ar['status']=='' && $ar['from']!='' &&  $ar['to']=='' ){
$this->db->where("t3.order_datetime >= '".$ar['from']."'");  
}
else if(!empty($ar) && $ar['pay']=='' && $ar['status']=='' && $ar['from']!='' &&  $ar['to']!='' ){
$this->db->where("t3.order_datetime >= '".$ar['from']."'");
  $this->db->where("t3.order_datetime <= '".$ar['to']."'");  
...
$this->db->where('ovd.status',$ar['status']);
...
$this->db->where('t3.payment_method',$ar['pay']);
}
else if(!empty($ar) && $ar['pay']!='' && $ar['status']=='' && $ar['from']=='' &&  $ar['to']!='' ){
$this->db->where('t3.payment_method',$ar['pay']);
$this->db->where("t3.order_datetime <= '".$ar['to']."'");  
...
public function getOrderdataForAdmin($ar){
...
$this->db->where("t3.order_datetime >= '".$ar['from']."'");  
  ...
  $this->db->where("t3.order_datetime >= '".$ar['from']."'");
  $this->db->where("t3.order_datetime <= '".$ar['to']."'");
//$this->db->where("t3.order_datetime BETWEEN '".$ar['from']."' AND '".$ar['to']."'");
  
}
else if(!empty($ar) && $ar['pay']=='' && $ar['status']!='' && $ar['from']=='' &&  $ar['to']=='' ){
$this->db->where('t3.order_status',$ar['status']);
...
$this->db->where('ovd.status',$ar['status']);
$this->db->where("t3.order_datetime >= '".$ar['from']."'");
  $this->db->where("t3.order_datetime <= '".$ar['to']."'");$this->db->where("t3.order_datetime <= '".$ar['to']."'");
}
...
public function getOrderVendorStatus($oid){
  $this->db->select('*');
      $this->db->from('order_vendor_driver as ovd');
      $this->db->where("ovd.order_id",$oid);     
      $query = $this->db->get();
      $row=$query->result_array();
      return $row;
}

public function getEmergencyList($id=NULL){

      $this->db->select('*');
      $this->db->from('emergency_request as er');
       $this->db->join('emergency_option as eo', 'er.option_val = eo.id', 'LEFT');
       $this->db->join('drivers as dr', 'er.driver_id = dr.id', 'LEFT');
     if($id){
      $this->db->where("er.vendor_id",$id);
      }     
           $this->db->group_by('er.order_id');
...
public function emergencyCnt($id=NULL){

      $this->db->select('*');
      $this->db->from('emergency_request as er');
      $this->db->where("er.action",0);
     if($id){
      $this->db->where("er.vendor_id",$id);     
     }
     $this->db->group_by('er.order_id');
...
public function generateinvoice($vid=NULL,$mid,$yid){

      $query = $this->db->query("SELECT oi.product_name,drivers.name_ar,drivers.name_en,oi.total_amount,oi.vendor_id,orders.created,orders.payment_method,orders.id FROM `orders` 
        left join order_vendor_driver on orders.id = order_vendor_driver.order_id  
        left join drivers on drivers.id = order_vendor_driver.driver_id 
        left join order_item as oi on oi.order_id=orders.id 
        where order_vendor_driver.status=3 and order_vendor_driver.vendor_id=".$vid." 
        and oi.vendor_id=".$vid." 
        and MONTH(orders.created)='".$mid."' 
        and YEAR(orders.created)='".$yid."'");
      $query = $query->result();
...
public function getVendorName($vid=NULL){

      $query = $this->db->query("SELECT * FROM `cats` where id =".$vid." ");
      $query = $query->result();
