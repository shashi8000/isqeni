...
public function generateinvoice($vid=NULL,$mid,$yid){
      ...
      $query = $this->db->query("SELECT oi.product_name,oi.total_amount,oi.vendor_id,orders.created,orders.payment_method FROM `orders` left join order_item as oi on oi.order_id=orders.id where oi.vendor_id=".$vid." and MONTH(created)='".$mid."' AND YEAR(created)='".$yid."'");
      $query = $query->result();
