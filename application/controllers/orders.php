...
/*
    ...
    if(@$_REQUEST['pay'] !='' && @$_REQUEST['pay'] !='0'){
      ...
      $query = $this->db->get();
  */
      $data['data'] = $this->data->getOrderdata($_REQUEST,$cat);
...
//*****************************************************************
  ...
  */
   ...
   public function driverdatain(){
      if($_REQUEST !=''){
        ...
        curl_setopt($ch, CURLOPT_POSTFIELDS,
          ...
          "userid=".$rows[0]['client_id']."&content=".lang("OrderConfirmedByTeam")."&title=Isqeni Admin Notification");
        curl_setopt($ch, CURLOPT_HTTPHEADER, array('Content-Type: application/x-www-form-urlencoded','secret:tcl1806'));
        curl_setopt($ch, CURLOPT_RETURNTRANSFER, true);
        $server_output = curl_exec ($ch);
        curl_close ($ch); 
  ...
  public function rejection($id){
      ...
      $this->load->view('customer/global/topbar');
      $this->load->view('customer/global/menu');
      $this->load->view('customer/pages/rejection',$data);
      $this->load->view('customer/global/footer');
      if(isset($_REQUEST['oid'])  && !empty($_REQUEST['oid'])){
            ...
            $this->db->select('*');
            $this->db->from('orders');
            $this->db->where('id',$_REQUEST['oid']);
            $query = $this->db->get();
            $rows=$query->result_array();

            if($_REQUEST['reason']=='first'){
              $reason = lang('OrderRejectByTeamOne');
            }else if($_REQUEST['reason']=='second'){
              $reason = lang('OrderRejectByTeamTwo');
            }else if($_REQUEST['reason']=='three'){
              $reason = lang('OrderRejectByTeamThree');
            }
             ...
             $vendor_id=$this->session->userdata('logged_in_customer');
              $vendor=$vendor_id['id'];
             // / $dataInsert['id'] = '';
              $dataInsert['vendor_id'] = $vendor;
              $dataInsert['order_id'] = $_REQUEST['oid'];
              $dataInsert['reason'] = $_REQUEST['reason'];
               $dataInsert['created_at'] = date('Y-m-d H:i:s');
              $this->db->insert('order_reject_vendors',$dataInsert);
  
              $dataUp=array('status'=>2);
              $this->db->where('order_id',$_REQUEST['oid']);
              $this->db->where('vendor_id',$vendor);
              $this->db->update('order_vendor_driver',$dataUp);                   
        ...
        curl_setopt($ch, CURLOPT_POSTFIELDS,
          ...
          "userid=".$rows[0]['client_id']."&content=".$reason."&title=Isqeni Admin Notification");
        curl_setopt($ch, CURLOPT_HTTPHEADER, array('Content-Type: application/x-www-form-urlencoded','secret:tcl1806'));
        curl_setopt($ch, CURLOPT_RETURNTRANSFER, true);
        $server_output = curl_exec ($ch);
        curl_close ($ch); 
