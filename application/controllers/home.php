...
class Home extends CI_Controller {
    ...
    public function getCatProducts2($catid , $area = 0) //  عرض المناطق من خلال الدولة
    {
       header('Content-Type: application/json; charset=utf-8');
        if($area == 0){
         $text = $this->data->get("products",FALSE,array("cat_id"=>$catid),FALSE,TRUE,array("id"=>"asc"),FALSE,array("id"=>"id","name"=>"name","name_en"=>"name_en","price"=>"price","min_charge"=>"min_charge","photo"=>"photo"));
        } else{
         $text = $this->data->get("products",FALSE,array("cat_id"=>$catid,"branch_id"=>$area),FALSE,TRUE,array("id"=>"asc"),FALSE,array("id"=>"id","name"=>"name","name_en"=>"name_en","price"=>"price","min_charge"=>"min_charge","photo"=>"photo"));
        }
       echo(json_encode($text));
    }
    public function getProduct($id,$userid=null) //  عرض المناطق من خلال الدولة
    {
       header('Content-Type: application/json; charset=utf-8');
           $text = $this->data->get("products",FALSE,array("id"=>$id),FALSE,FALSE,FALSE,FALSE,array("id"=>"id","name"=>"name","name_en"=>"name_en","photo"=>"photo","text2"=>"FeedData","text2_en"=>"FeedData_en","amount"=>"amount","price"=>"price","cat_id"=>"cat_id","active"=>"active","offer"=>"offer",'min_charge'=>'min_charge'));
    ...
    {
        header('Content-Type: application/json; charset=utf-8');
        $t = $this->data->get("cats",FALSE,array("parent"=>$parent),FALSE,TRUE,array("id"=>"asc"));
...
//******************************************************************************************
    ...
    public function orderStatus($id)   //
    {
      header('Content-Type: application/json; charset=utf-8');
        ...
        $text = $this->data->get("orders",FALSE,array("id"=>$id),1);
        if($text['status'] == 0){ $text['status'] = "pending";}
        elseif($text['status'] == 1){ $text['status'] = "shipped";}
        elseif($text['status'] == 2){ $text['status'] = "delivered";}
    ...
    public function getNotisNumbers($user = 0 , $ios = 0) // عرض التنبيهات
    {
       ...
       if ( $user == 0 )
       {
        ...
        $t = $this->data->countTable("cart",["client_id"=>$user,"status"=>1]);
       }
    ...
    public function questionnaire($ios = 0)
    {
       header('Content-Type: application/json; charset=utf-8');
           ...
           $x = $this->data->insert("question",$theData) ;
         if($x)
         {
           $z = ['message_en'=>'delivered',
                 'message_ar'=>'تم الأستبيان',
                 'status'=>'true'
                 ];
           if($ios == 1)
           {
    ...
    public function complaint($ios = 0)
    {
       header('Content-Type: application/json; charset=utf-8');
           ...
           $x = $this->data->insert("complaints",$theData) ;
         if($x)
         {
           $z = ['message_en'=>'Your message has been sent',
                 'message_ar'=>'تم ارسال رسالتك بنجاح'];
           if($ios == 1)
           {
              $text = array("res"=>$z) ;
           }else {
    ...
    public function getNotify($ios = 0) // عرض التنبيهات
    {
       header('Content-Type: application/json; charset=utf-8');
       $client = $this->input->get("client_id");
       $date = $this->input->get("date");
       $x = $this->db->get_where("notifications",["client_id"=>$client,"ndate >"=>$date])->result_array();
       if($x)
         {
           if($ios == 1)
           {
    ...
    public function PayFortSendRequest(){
        
        $device_id = 
        "<script>
        FortSdk.getDeviceId(context);
        </script>";
        $requestParams = array(
            'service_command' => 'PURCHASE',
            'access_code' => 'uTecTUThAnGbSAUzoJRo',
            'merchant_identifier' => 'bPnAGuAb',
            'language' => 'ar',
            'device_id' => $device_id,
        );
