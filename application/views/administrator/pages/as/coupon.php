    ...
    public function add_voucher(){
		...
		$data = $this->input->post('Cform');
			if($data !=''){
				...
				foreach($rows as $row){
					$productID=$row->id;
						$sqlDelete = "delete from coupon_products where product_id=".$productID;
						$this->db->query($sqlDelete);

						$status = '1';
						$sqlcoupon = "insert into coupon_products (product_id, coupon_id, status) values ('$productID', '$couponID', '$status')";
